...
class RetryDemo(Spider):
    ...
    async def acticle_list(self, response):
        ...
        if '验证码' in html:
            ...
            res = requests.get(over_url, headers=self.headers)
            code_res = ocr.classification(res.content)
            time.sleep(2)
            check_url = 'https://oversea.cnki.net/kns/Brief/CheckCode'
            data = {
                "vericode": code_res,
                "corgin": "OverMaxSearchCount"
            }
            resp = requests.post(check_url, headers=self.headers, data=data)
            print(f'验证输入成功{resp.status_code}')
